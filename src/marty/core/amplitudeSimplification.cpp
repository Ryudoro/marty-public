...
bool mayBeSimplified(csl::Expr const &expr)
{
    if (IsOfType<mty::Propagator>(expr))
        return false;
    if (csl::IsPow(expr) and IsOfType<mty::Propagator>(expr[0])
...
void findExternalAbbreviation(csl::Expr &expr)
{
    csl::ForEachNode(expr, [&](csl::Expr &sub) {
        if (csl::IsProd(sub)) {
            ...
            if (!external.empty()) {
                sub = prod_s(
                    csl::Abbrev::makeAbbreviation("EXT", prod_s(external)),
                    csl::Refreshed(sub));
            }
        }
    });
...
void abbreviateIntegral(csl::Expr &res)
{
    if (csl::IsSum(res)) {
        ...
        for (const auto &arg : res) {
            ...
            if (size == 0 and !csl::IsNumerical(arg)
                and !IsOfType<FeynmanIntegral>(arg)) {
                integral = false;
                break;
            }
            else if (size > 0 and !IsOfType<FeynmanIntegral>(arg)) {
        ...
        if (integral) {
            res = csl::Abbrev::makeAbbreviation("INT", res);
        }
        else
...
void abbreviateAll(csl::Expr &res)
{
    ...
    if (csl::IsSum(res)) {
        ...
        for (auto &arg : res) {
            ...
            else if (csl::Abbrev::getFreeStructure(arg->getIndexStructure())
                             .size()
                         == 0
                     and !csl::IsNumerical(arg) and arg != csl::DMinko) {
        ...
        csl::Expr ext = (externalLegs.size() > 0)
                            ? csl::Abbrev::makeAbbreviation(
                                "EXT", csl::prod_s(externalLegs, true))
                            : CSL_1;
        csl::Expr mom = (momenta.size() > 0) ? csl::Abbrev::makeAbbreviation(
                            "P", csl::prod_s(momenta, true))
                                             : CSL_1;
        csl::Expr factor;
        if (factors.size() > 1
...
bool findAbbreviations(csl::Expr &res)
{
    ...
    for (size_t i = 0; i != res->size(); ++i) {
        if ((*res)[i]->getPrimaryType() != csl::PrimaryType::Numerical
            ...
            and csl::AllOfLeafs((*res)[i], [](const csl::Expr &el) {
                    return (
                        el->getPrimaryType() == csl::PrimaryType::Numerical
                        or (el->getPrimaryType() == csl::PrimaryType::Literal
    ...
    if (constants.empty())
        return false;
    if (constants.size() > 1 or res->getArgument(constants[0])->size() > 0) {
...
color::ColorSpace const *isColorStructure(csl::Expr const &expr)
{
    if (csl::AnyOfLeafs(expr, [&](csl::Expr const &sub) {
            return IsOfType<PolarizationField>(sub) or sub->getName() == "EXT";
        })) {
        return nullptr;
    }
...
csl::IndexStructure colorStructure(csl::Expr const &        node,
...
bool factorIndicial(csl::Expr &res)
{
    auto termAndFactor
        ...
        if (csl::IsProd(expr)) {
            ...
            return (expr->isIndexed()) ? std::make_pair(CSL_1, expr)
        ...
        for (size_t i = 0; i != termsAndFactors.size(); ++i) {
            ...
            for (size_t j = i + 1; j != termsAndFactors.size(); ++j) {
                if (termsAndFactors[j].first != CSL_0
                    && termsAndFactors[j].second
                           == termsAndFactors[i].second) {
...
csl::Expr
colorAbbreviation(std::vector<csl::Expr> const &                tensors,
    ...
    while (!indicesLeft.empty()) {
        ...
        if (not csl::IsProd(toAbbreviate)) {
            if (toAbbreviate->isIndexed())
                res *= csl::Abbrev::makeAbbreviation("Color", toAbbreviate);
            else
                res *= toAbbreviate;
        }
        else {
            ...
            res *= toAbbreviate;
            res *= csl::Abbrev::makeAbbreviation("Color", csl::prod_s(color));
        }
    }
    return res;
}
...
findStructureMatches(std::vector<csl::IndexStructure> const &structures)
{
...
bool expandInProd(csl::Expr &prod,
                  std::function<csl::IndexStructure(csl::Expr const &)> const
                      &structureGetter)
{
    ...
    for (size_t i = 0; i != prod->size(); ++i) {
        if (auto structure = structureGetter(prod[i]); !structure.empty()) {
            pos.push_back(i);
    ...
    for (size_t i = 0; i != toExpand.size(); ++i) {
        ...
        HEPAssert(prod[pos[positions[0]]] == CSL_1,
                  ...
                  "Argument " + toString(prod[pos[positions[0]]])
                      + " should be "
                        "1..") prod[pos[positions[0]]]
            = csl::Expanded(csl::prod_s(args));
    }
    csl::Refresh(prod);
    return true;
}
...
bool expandColorIndices(csl::Expr &res)
{
    ...
    for (auto c : colors) {
        csl::ForEachNode(res, [&](csl::Expr &prod) {
            if (csl::IsProd(prod))
                expanded = expandInProd(prod,
                                        [c](csl::Expr const &sub) {
...
bool findColorAbbreviation(csl::Expr &expr)
{
    ...
    csl::ForEachNodeCut(expr, [&](csl::Expr &sub) {
        ...
        for (auto &arg : sub)
            if (auto space = isColorStructure(arg); space) {
...
static bool hasCommonIndex(csl::Expr const & A,
                           csl::Expr const & B,
                           csl::Space const *space = nullptr)
{
    csl::IndexStructure const &Aindex = csl::IsIndicialTensor(A)
                                            ? A->getIndexStructureView()
                                            : A->getIndexStructure();
    csl::IndexStructure const &Bindex = csl::IsIndicialTensor(B)
...
bool expandMinkoMetric(csl::Expr &expr)
{
    ...
    auto isReceiver
        = [&](csl::Expr const &emitter, csl::Expr const &receiver) {
...
bool simplifyEpsilonInProd(csl::Expr &prod, csl::IndexStructure &indices)
{
    ...
    for (size_t i = 0; i != indices.size(); ++i)
        for (size_t j = i + 1; j < indices.size(); ++j) {
            csl::Expr replaced = csl::Swapped(prod, indices[i], indices[j]);
            if (replaced == prod) {
                prod = CSL_0;
                return true;
            }
...
bool expandGammaMatrices(csl::Expr &expr)
{
    ...
    bool toExpand = csl::AnyOfNodes(expr, [&](csl::Expr const &sub) {
        return csl::IsSum(sub) and sub.get() != expr.get()
...
bool simplifyFermionChains(csl::Expr &expr)
{
    bool gamma = csl::AnyOfLeafs(expr, [&](csl::Expr const &sub) {
        return csl::IsIndicialTensor(sub) && dirac4.isGammaTensor(sub);
    });
    if (!gamma)
        return false;
    bool         simplified = false;
...
void expandForFermionOrdering(csl::Expr &expr)
{
    auto isEmitter = [&](csl::Expr const &emitter) {
        if (csl::IsIndicialTensor(emitter)) {
            ...
            return std::any_of(
                indices.begin(), indices.end(), [&](csl::Index const &index) {
...
getMomentumReplacement(std::vector<mty::QuantumField> const &insertions,
                       std::vector<csl::Tensor> const &      momenta,
                       size_t                                posReplaced)
{
    ...
    if (insertions[posReplaced].isIncoming())
        momentumSum *= CSL_M_1;
    return {csl::Tensor(momenta[posReplaced])(mu), momentumSum};
}

void replaceMomentum(csl::Expr &                           init,
                     std::vector<mty::QuantumField> const &insertions,
                     std::vector<csl::Tensor> const &      momenta,
                     size_t                                posReplaced)
{
    auto [p, momentumEquivalence]
        = getMomentumReplacement(insertions, momenta, posReplaced);
...
void applyEOM(csl::Expr &ampl, std::vector<FermionEOMData> &onShellFermions)
{
    ...
    csl::ForEachNode(ampl, [&](csl::Expr &sub) {
        if (csl::IsProd(sub)) {
            ...
            sgl::TensorSet tset{dirac4.gamma_chir,
                                dirac4.C_matrix,
                                dirac4.P_L,
                                dirac4.P_R,
                                {}};
              ...
              std::vector<csl::Tensor> const &      momenta)
{
    ...
    csl::ForEachLeaf(ampl, [&](csl::Expr const &sub) {
        ...
        if (field.getQuantumParent()->getSpinDimension() == 2
            and field.isOnShell()) {
            onShellFermions.push_back(
                {field.getPoint(),
                 field.getMass(),
                 field.getIndexStructureView().back(),
                 field.isIncomingParticle() or field.isOutgoingParticle()});
        }
    });
              ...
              Mode                                  mode)
{
    ...
    do {
        ...
        if (loop == 0 && mode != FeynmanRule && mode != SquaredAmplitude
    ...
    // Warning here: for now the addLocalTerms() method needs the integrals to
    // be NOT FACTORED. Beware not to call factor() before addLocalTerms() then
    ...
    if (!factors.empty()) {
        ...
        csl::ForEachNode(factor, [&](csl::Expr &sub) {
            if (IsOfType<Propagator>(sub))
                sub = csl::Factored(csl::Expanded(
                    csl::Evaluated(sub, csl::eval::abbreviation)));
        });
        if (mty::option::searchAbreviations
