...
{
    switch (type) {
    case mty::gauge::GroupType::U1:
        fout << "U1";
        break;
    case mty::gauge::GroupType::SU:
        fout << "SU";
        break;
    case mty::gauge::GroupType::SO:
        fout << "SO";
        break;
    case mty::gauge::GroupType::Sp:
        fout << "Sp";
        break;
    case mty::gauge::GroupType::E6:
        fout << "E6";
        break;
    case mty::gauge::GroupType::E7:
        fout << "E7";
        break;
    case mty::gauge::GroupType::E8:
        fout << "E8";
        break;
    case mty::gauge::GroupType::F4:
        fout << "F4";
        break;
    case mty::gauge::GroupType::G2:
        fout << "G2";
        break;
    default:
...
{
    CallHEPError(mty::error::NotImplementedError,
                 "Getting specific spinor invariant tensor for a wrong "
                 "algebra.");
    return nullptr;
}
...
void GaugedGroup::setCouplingConstant(const csl::Expr &t_coupling)
{
    HEPAssert(t_coupling->getType() == csl::Type::Constant,
              ...
              "Expression for coupling constant of gauge group should be "
                  + (string) "a constant not a "
                  + toString(t_coupling->getType()));
    coupling = t_coupling;
}
...
mty::Generator GaugedGroup::getGenerator(const Irrep &field)
{
    if (not hasGeneratorForRep(field)) {
        ...
        setGeneratorProperties(field, generator[field]);
    }
...
csl::Expr
GaugedGroup::getEffectiveQuadraticCasimir(mty::Irrep const &rep) const
{
    auto      defining = group->getAlgebra()->getDefiningRep();
    csl::Expr res
        = normalization(getType())
          * group->getAlgebra()->getQuadraticCasimir(rep.getHighestWeight())
          / group->getAlgebra()->getQuadraticCasimir(
              defining.getHighestWeight())
...
void GaugedGroup::setGeneratorProperties(Irrep const &, mty::Generator &)
{
}

std::string getGeneratorName(std::string const &     init,
...
void GaugedGroup::generateRepGenerator(const Irrep &irrep)
{
    ...
    generator[irrep]        = mty::Generator(
        ...
        irrep,
        getGeneratorName(
            nameGenerator, getName(), irrep.getHighestWeight().getLabels()),
                                        ...
                                        Tensor &             point)
{
    ...
    csl::Expr gaugeCoupling = -CSL_I * intfraction_s(num, denom) * coupling
                              * (*boson)({mu}, point);
...
csl::Expr
NonAbelianGauged::covariantDerivative(QuantumFieldParent & field,
                                      const Index &        mu,
                                      const vector<Index> &fieldIndices,
                                      Tensor &             point)
{
    ...
    csl::Expr gaugeCoupling
        = (-CSL_I) * coupling * (*boson)({+A, mu}, point) * T({+A, a, b});
...
void NonAbelianGauged::setGeneratorProperties(Irrep const &   rep,
                                              mty::Generator &T)
{
    ...
    if (space == adjointSpace)
        res *= -1;
    T->addSelfContraction(T({A, i, j}), T({B, j, i}), res * delta_adj({A, B}));
    T->addTraceLessNess(space);

    if (space != adjointSpace) {
        T->setComplexProperty(csl::ComplexProperty::Complex);
        T->addHermitianProperty(space, T({A, i, j}), T({A, j, i}));
        T->addSelfContraction(f({A, B, C}),
                              T({C, i, j}),
...
bool NonAbelianGauged::isAdjointGenerator(mty::Generator const &tensor) const
{
    return tensor.get() == f.get();
}

int NonAbelianGauged::getGeneratorDim(mty::Generator const &tensor) const
{
...
void SUGauged::setGeneratorProperties(Irrep const &rep, mty::Generator &T)
{
    ...
    if (!isAdjointGenerator(T)) {
        ...
        // Hermitian generator
        T->addHermitianProperty(T->getSpace()[1], T({A, i, j}), T({A, i, j}));
        T->addComplexProperty(T({A, i, j}), T({A, j, i}));
        // Projection relation
        // T^A_ij T^A_kl = 1/2 * ( d_il*d_jk - 1/N d_ij*d_kl )
        if (rep == group->getAlgebra()->getDefiningRep())
            T->addSelfContraction(T({A, i, j}),
                                  T({A, k, l}),
        ...
        if (N == 2 and getGeneratorDim(T) == orderL + 1) {
            T->setTensor(
                csl::highdtensor_from_args_s({CSL_1 / 2 * pauli::sigma1,
                                              CSL_1 / 2 * pauli::sigma2,
                                              CSL_1 / 2 * pauli::sigma3}));
        }
        else if (N == 3 and getGeneratorDim(T) == orderL + 1) {
            T->setTensor(csl::highdtensor_from_args_s(
...
void SOGauged::setGeneratorProperties(mty::Irrep const &rep, mty::Generator &T)
{
    NonAbelianGauged::setGeneratorProperties(rep, T);
    if (!isAdjointGenerator(T)) {
        const Space *space     = T->getSpace()[1];
        ...
        // Symmetric generator
        T->addTransposedProperty(space, T({A, i, j}), T({A, i, j}));
...
void SOGauged::initDTilde()
{
    ...
    d_tilde->addSelfContraction(d_tilde(indices),
                                d_tilde(indices),
                                csl::pow_s(2, -m) * csl::factorial(2 * m)
                                    / csl::int_s(4));
}
...
void SpGauged::setGeneratorProperties(mty::Irrep const &rep, mty::Generator &T)
{
    NonAbelianGauged::setGeneratorProperties(rep, T);
    if (!isAdjointGenerator(T)) {
        const Space *space     = T->getSpace()[1];
        ...
        T->addTransposedProperty(space, T({A, i, j}), T({A, j, i}));
...
void E6Gauged::setGeneratorProperties(mty::Irrep const &rep, mty::Generator &T)
{
    NonAbelianGauged::setGeneratorProperties(rep, T);
    if (!isAdjointGenerator(T)) {
        const Space *space     = T->getSpace()[1];
...
void E6Gauged::generateRepGenerator(mty::Irrep const &irrep)
{
    const Space *fieldSpace = group->getVectorSpace(irrep);
    if (fieldSpace == adjointSpace) {
        ...
        csl::Tensor d("d", {fieldSpace, fieldSpace, fieldSpace});
        d->setComplexProperty(csl::ComplexProperty::Real);
        d->setFullySymmetric();

        d->addSelfContraction(d({i, j, k}), d({l, +j, +k}), delta({i, l}));
        d->addSelfContraction(
            d({i, j, k}), d({+k, +j, l}), alpha * delta({i, l}));
        d_invariant[irrep] = d;
    }
...
void E7Gauged::setGeneratorProperties(mty::Irrep const &rep, mty::Generator &T)
{
    NonAbelianGauged::setGeneratorProperties(rep, T);
    if (!isAdjointGenerator(T)) {
        const Space *space     = T->getSpace()[1];
        ...
        if (rep == group->getAlgebra()->getDefiningRep())
            T->addSelfContraction(
                T({A, i, j}),
                T({A, k, l}),
                a / 24 * g({i, l}) * g({j, k}) + a / 24 * g({i, k}) * g({j, l})
                    - 1 / (alpha * 12) * d_invariant[rep]({i, j, k, l}));
    }
}

void E7Gauged::generateRepGenerator(mty::Irrep const &irrep)
{
    const Space *fieldSpace = group->getVectorSpace(irrep);
    if (fieldSpace == adjointSpace) {
        csl::Index  i = fieldSpace->generateIndex();
        ...
        csl::Index  k = fieldSpace->generateIndex();
        csl::Index  l = fieldSpace->generateIndex();
        csl::Tensor d("d", {fieldSpace, fieldSpace, fieldSpace, fieldSpace});
        d->setComplexProperty(csl::ComplexProperty::Real);
        d->setFullySymmetric();
        d_invariant[irrep] = d;
    }
...
void F4Gauged::setGeneratorProperties(mty::Irrep const &rep, mty::Generator &T)
{
    NonAbelianGauged::setGeneratorProperties(rep, T);
    if (!isAdjointGenerator(T)) {
        const Space *space     = T->getSpace()[1];
...
void F4Gauged::generateRepGenerator(mty::Irrep const &irrep)
{
    const Space *fieldSpace = group->getVectorSpace(irrep);
    if (fieldSpace == adjointSpace) {
        csl::Index  i     = fieldSpace->generateIndex();
...
void G2Gauged::setGeneratorProperties(mty::Irrep const &rep, mty::Generator &T)
{
    if (!isAdjointGenerator(T)) {
        const Space *space     = T->getSpace()[1];
        csl::Index   A         = adjointSpace->generateIndex();
...
void G2Gauged::generateRepGenerator(mty::Irrep const &irrep)
{
    const Space *fieldSpace = group->getVectorSpace(irrep);
    if (fieldSpace == adjointSpace) {
        csl::Index     i     = fieldSpace->generateIndex();
        ...
        csl::Tensor    delta = fieldSpace->getDelta();
        mty::Generator f(
            this, irrep, "f", {fieldSpace, fieldSpace, fieldSpace});
        f->setComplexProperty(csl::ComplexProperty::Real);
        f->setFullyAntiSymmetric();
        f->addSelfContraction(
            f({i, j, k}), f({+k, +j, l}), alpha * delta({i, l}));
        f_invariant[irrep] = f;
    }
    NonAbelianGauged::generateRepGenerator(irrep);
}
...
unique_ptr<GaugedGroup> createGaugedGroup(SemiSimpleGroup *group,
                                          Gauge *          gauge,
                                          ...
                                          const csl::Expr &coupling)
{
    ...
    case group::Type::U1:
        if (coupling != CSL_UNDEF)
            ...
            gauged = make_unique<U1Gauged>(group, gauge, nameBoson, addGhost);
        break;
    ...
    case group::Type::SU:
        if (coupling != CSL_UNDEF)
            ...
            gauged = make_unique<SUGauged>(group, gauge, nameBoson, addGhost);
        break;
    ...
    case group::Type::SO:
        if (coupling != CSL_UNDEF)
            ...
            gauged = make_unique<SOGauged>(group, gauge, nameBoson, addGhost);
        break;
    ...
    case group::Type::Sp:
        if (coupling != CSL_UNDEF)
            ...
            gauged = make_unique<SpGauged>(group, gauge, nameBoson, addGhost);
        break;
    ...
    case group::Type::E6:
        if (coupling != CSL_UNDEF)
            ...
            gauged = make_unique<E6Gauged>(group, gauge, nameBoson, addGhost);
        break;
    ...
    case group::Type::E7:
        if (coupling != CSL_UNDEF)
            ...
            gauged = make_unique<E7Gauged>(group, gauge, nameBoson, addGhost);
        break;
    ...
    case group::Type::E8:
        if (coupling != CSL_UNDEF)
            ...
            gauged = make_unique<E8Gauged>(group, gauge, nameBoson, addGhost);
        break;
    ...
    case group::Type::F4:
        if (coupling != CSL_UNDEF)
            ...
            gauged = make_unique<F4Gauged>(group, gauge, nameBoson, addGhost);
        break;
    ...
    case group::Type::G2:
        if (coupling != CSL_UNDEF)
            ...
            gauged = make_unique<G2Gauged>(group, gauge, nameBoson, addGhost);
        break;
