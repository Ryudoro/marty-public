...
static mty::QuantumField createField(mty::Insertion const &ins,
...
static bool
requirementsSatisfied(std::vector<OpInsertion> const &         insertions,
...
getMagneticCoupling(DiracCoupling coupling, csl::Index alpha, csl::Index beta)
{
    ...
    case DiracCoupling::L:
        return {{CSL_1, id({alpha, beta})}, {CSL_M_1, g5({alpha, beta})}};
    case DiracCoupling::R:
        return {{CSL_1, id({alpha, beta})}, {CSL_1, g5({alpha, beta})}};
    default:
    ...
    if (A->getIndexStructureView().size() > 1) {
        ...
        HEPAssert(group,
            ...
            HEPAssert(qf(psi)->getGroupIrrep(group).getDim() > 1,
                      ...
                      "Particle " + psi->getName()
                          + " has no representation to "
                            "couple to "
                          + A->getName() + " in operator.") auto T
            = model.getGenerator(group, psi);
...
static std::vector<Wilson>
buildMagneticOperator(Model const &      model,
                      ...
                      DiracCoupling      coupling)
{
    ...
    csl::Expr A = vectorBoson.field.getQuantumParent()->getInstance();
    ...
    std::vector<Wilson> diracCoupling
        = getMagneticCoupling(coupling, alpha, beta);
...
static std::array<OpInsertion, 3>
getMagneticInsertions(std::vector<OpInsertion> const &insertions, bool swapped)
{
...
static void addMagneticContribution(std::vector<Wilson> &contributions,
                                    ...
                                    DiracCoupling        coupling)
{
    std::vector<Wilson> newContributions = buildMagneticOperator(
        model, leftFermion, rightFermion, vectorBoson, momentum, coupling);
    for (const auto &contrib : newContributions) {
        contributions.push_back({contrib.coef.getCoefficient() * factor,
                                           ...
                                           DiracCoupling    coupling)
{
    ...
    if (fermionOrder.empty())
        fermionOrder = defaultFermionOrder(wilsons.kinematics.getInsertions());
    bool                     swapped    = areSwapped(fermionOrder);
    std::vector<OpInsertion> insertions = getOpInsertions(wilsons.kinematics);
    ...
    std::vector<Wilson> res;
    res.reserve(6);
    addMagneticContribution(res,
                            leftFactor,
                            model,
                            leftFermion,
                            rightFermion,
                            vectorBoson,
                            leftFermion.momentum,
                            coupling);
    addMagneticContribution(res,
                            rightFactor,
                            model,
    ...
    addMagneticContribution(res,
                            ...
                            rightFermion,
                            vectorBoson,
                            vectorBoson.momentum,
                            coupling);

    return res;
}

static void setUpDim6FermionOrder(WilsonSet const & wilsons,
                                  std::vector<int> &fermionOrder)
{
    if (fermionOrder.empty()
        ...
        CallHEPError(mty::error::TypeError,
                     "Expecting a fermion order for 4 fermions, vector of "
                     "size "
...
static std::array<csl::Expr, 4>
getExternalDim6Fermions(std::vector<OpInsertion> const &insertions,
                        std::vector<int> const &        fermionOrder)
{
    return {fermionExpr(insertions[fermionOrder[0]], true),
...
static std::vector<Wilson> getDiracCurrent(DiracCoupling     current,
                                           csl::Index const &a,
                                           csl::Index const &b,
                                           csl::Index const &mu,
                                           csl::Index const &nu = csl::Index())
{
    ...
    case DiracCoupling::L:
        return {{CSL_1, delta({a, b})}, {CSL_M_1, gamma5({a, b})}};
    case DiracCoupling::R:
        return {{CSL_1, delta({a, b})}, {CSL_1, gamma5({a, b})}};
    ...
    case DiracCoupling::VL:
        return {{CSL_1, gamma({mu, a, c}) * delta({c, b})},
    ...
    case DiracCoupling::TL:
        return {{CSL_1, sigma({mu, nu, a, c}) * delta({c, b})},
...
static std::vector<Wilson> getDiracCurrent(DiracCoupling c1,
                                           DiracCoupling c2,
                                           csl::Expr &   psi1,
                                           csl::Expr &   psi2,
                                           csl::Expr &   psi3,
                                           csl::Expr &   psi4)
{
    ...
    for (const auto &c1 : w1) {
        for (const auto &c2 : w2) {
            res.push_back({c1.coef.getCoefficient() * c2.coef.getCoefficient(),
...
static std::pair<size_t, size_t> findTwoIdenticalReps(mty::Irrep const &rep1,
...
static bool areIdenticalReps(mty::Irrep const &rep1,
                             mty::Irrep const &rep2,
                             mty::Irrep const &rep3,
                             mty::Irrep const &rep4)
{
    return rep1 == rep2 && rep1 == rep3 && rep1 == rep4 && rep1.getDim() > 1;
}
...
csl::Index &groupIndex(Model const &model, Group const *group, csl::Expr &psi)
{
    ...
    CallHEPError(mty::error::RuntimeError,
                 "Index for field " + toString(psi) + " in group "
                     + group->getName() + " not found.") static csl::Index def;
    return def;
}

static csl::Expr getTrivialColorCouplings(Model const &model,
                                          csl::Expr &  psi1,
                                          csl::Expr &  psi2,
                                          csl::Expr &  psi3,
                                          csl::Expr &  psi4)
...
static ColorCoupling
getColorCoupling(std::vector<ColorSpec> const &colorCouplings,
...
static csl::Expr getColorCoupling(ColorCoupling     coupling,
                                  csl::Index const &a1,
                                  csl::Index const &a2,
                                  csl::Index const &a3,
                                  csl::Index const &a4,
...
static csl::Expr
getGeneralColorCouplings(Model const &                 model,
                         ...
                         csl::Expr &                   psi4)
{
    ...
    for (size_t i = 0; i != model.getGauge()->size(); ++i) {
        ...
        if (identical) {
            ...
            ColorCoupling current
                = getColorCoupling(colorCouplings, group->getName());
...
static csl::Expr getColorCurrent(Model const &                 model,
                                 std::vector<ColorSpec> const &colorCouplings,
                                 csl::Expr &                   psi1,
                                 csl::Expr &                   psi2,
                                 csl::Expr &                   psi3,
                                 csl::Expr &                   psi4)
{
    csl::Expr trivialCouplings
        = getTrivialColorCouplings(model, psi1, psi2, psi3, psi4);
                   ...
                   std::vector<int>              fermionOrder)
{
    ...
    auto [psi1, psi2, psi3, psi4]
        = getExternalDim6Fermions(insertions, fermionOrder);
    std::vector<Wilson> diracCouplings
        = getDiracCurrent(leftCurrent, rightCurrent, psi1, psi2, psi3, psi4);
    csl::Expr colorCurrent
        = getColorCurrent(model, colorCouplings, psi1, psi2, psi3, psi4);
    csl::Expr remnant = csl::prod_s({psi1, psi2, psi3, psi4, colorCurrent});
    for (auto &coef : diracCouplings)
        coef.op.setExpression(coef.op.getExpression()
                              * csl::DeepCopy(remnant));
                                       ...
                                       DiracCoupling    current)
{
    ...
    std::vector<Wilson> diracCouplings
        = getDiracCurrent(current, alpha, beta, mu);
    csl::Expr generator = getMagneticGenerator(model, psi_star, psi, A);
    std::vector<Wilson> res;
    res.reserve(diracCouplings.size());
    for (const auto &coup : diracCouplings)
        res.push_back(
            {coup.coef.getCoefficient(),
             coup.op.getExpression() * psi_star * generator * psi * A});
    return res;
}
