...
class GeneratorParent : public csl::TensorParent {
  ...
  public:
    template <class... Args>
    GeneratorParent(mty::GaugedGroup *t_group,
...
{
    return csl::make_shared<GeneratorParent>(std::forward<Args>(args)...);
}

template <class... Args>
csl::Expr generatorelement_s(Args &&... args)
{
    return csl::make_shared<GeneratorElement>(std::forward<Args>(args)...)
...
class Generator : public std::shared_ptr<GeneratorParent> {
  ...
  public:
     ...
     */
    Generator(mty::GaugedGroup * t_group,
              mty::Irrep const & t_irrep,
