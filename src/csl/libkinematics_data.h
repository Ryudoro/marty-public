...
inline void replaceKinematicsMarker(std::string &text,
...
inline void print_libkinematics_hdata(std::ostream &out,
...
public:
    ...
    KinematicsCalculator(std::size_t incoming, std::size_t outgoing, const std::vector<double>& masses, double s, int ps_dim);
    ...
    virtual double compute_phase_space_factor(std::vector<double> kin_params) = 0;
    virtual void initialize_constants();

    virtual void set_mass(std::size_t idx, double m);

    void set_s(double s);

    const std::vector<Momentum>& get_momenta() const;
    const Momentum& get_momentum(std::size_t idx) const;

    double get_mass(std::size_t idx) const;
    const std::vector<double>& get_masses() const;

    int get_phase_space_dim() const;

protected:
    std::size_t m_incoming, m_outgoing;
...
public:
    ...
    void set_mass(std::size_t idx, double m) override;
...
private:
    const Rotation m_rot_pi {0, std::numbers::pi_v<double>, 0};
    double m_glob_fact;
};
...
public:
    ...
    Kinematics(std::size_t incoming, std::size_t outgoing, double sqrt_s, param_t *params);
    ...
    void set_ps_ind(std::function<bool(param_t*, const std::vector<double>&)>& ps_ind);

    void reset_ps_ind();

    const Momentum& get_momentum(std::size_t id) const;
    ...
    static bool DEFAULT_PS_IND(param_t*, const std::vector<double>&) { return true; }
    static bool DEFAULT_PS_IND_13(param_t* p, const std::vector<double>& masses);

private:
    std::size_t m_incoming, m_outgoing;
    param_t *m_params;
    std::function<bool(param_t*, const std::vector<double>&)> m_ps_ind;
    ...
    replaceKinematicsMarker(text, "@NAMESPACE@", libraryNamespace);
    out << text;
}

inline void print_libkinematics_cppdata(std::ostream &out,
                                        std::string const &libraryNamespace)
{
    std::string text = R"MTYNUM(#include "kinematics.h"
...
Momentum &Momentum::operator+=(const Momentum &rhs) {
    for (std::size_t i=0; i<this->m_components.size(); ++i)
        (*this)[i] += rhs[i];
    return *this;
}
...
std::unique_ptr<LorentzTransformation> Rotation::inverse() const {
    ...
    for (std::size_t i = 0; i < new_components.size(); i++) {
        for (std::size_t j = 0; j < new_components[0].size(); j++) {
...
Kinematics::Kinematics(std::size_t incoming, std::size_t outgoing, double sqrt_s, param_t *params)
...
void Kinematics::update(const std::vector<double>& kin_params, bool bypass_range_checks) {
    ...
    std::size_t n = m_incoming + m_outgoing;
...
void Kinematics::set_incoming_masses(const std::vector<double> &masses) {
    ...
    for (std::size_t i = 0; i < m_incoming; i++) {
...
void Kinematics::set_outgoing_masses(const std::vector<double> &masses) {
    ...
    for (std::size_t i = 0; i < m_outgoing; i++) {
...
const Momentum& Kinematics::get_momentum(std::size_t idx) const {
...
KinematicsCalculator::KinematicsCalculator(std::size_t incoming, std::size_t outgoing, const std::vector<double>& masses, double s, int ps_dim)
...
void KinematicsCalculator::initialize_constants() {
    for (std::size_t i = 0; i < m_masses.size(); i++) {
        m_masses_sq[i] = pow(m_masses[i], 2);
    }
}

void KinematicsCalculator::set_mass(std::size_t idx, double m) {
    m_masses[idx] = m;
    initialize_constants();
}
...
const Momentum& KinematicsCalculator::get_momentum(std::size_t idx) const {
    return m_momenta[idx];
}

double KinematicsCalculator::get_mass(std::size_t idx) const {
    return m_masses[idx];
}
...
void KinematicsCalculator12::set_mass(std::size_t idx, double m) {
    if (idx == 0) {
        m_s = m * m;
    }
    m_masses[idx] = m;
...
void KinematicsCalculator12::initialize_constants() {
    ...
    m_phase_space_factor = b23 / (16 * std::numbers::pi_v<double> * m_1);
}
...
void KinematicsCalculator13::set_mass(std::size_t idx, double m) {
    if (idx == 0) {
        m_s = m * m;
    }
    m_masses[idx] = m;
...
void KinematicsCalculator13::initialize_constants() {
    ...
    m_glob_fact = m_masses[0] / (8 * pow(2 * std::numbers::pi_v<double>, 3));
}
...
void KinematicsCalculator22::initialize_constants() {
    ...
    m_phase_space_factor = b34 / (32 * std::numbers::pi_v<double> * m_s * b12);
}
...
void KinematicsCalculator23::compute_momenta(std::vector<double> kin_params) {
    ...
    auto boost = LorentzBoost(gamma, 3);
    auto rot = Rotation(0, std::numbers::pi_v<double> + acos(c_theta), 0);
...
std::vector<std::pair<double, double>> KinematicsCalculator23::compute_kinematic_limits() const {
    ...
    return {{t_min, t_max}, {-1, 1}, {-1, 1}, {0, 2 * std::numbers::pi_v<double>}};
}
...
void KinematicsCalculator23::initialize_constants() {
    ...
    m_momenta[0] = std::move(Momentum(m_masses[0], p_in, {0, 0, 1}));
    m_momenta[1] = std::move(Momentum(m_masses[1], p_in, {0, 0, -1}));
    m_glob_fact = 1 / (128 * pow(2 * std::numbers::pi_v<double>, 4) * m_s * b12);
}
...
//     auto rot_56 = Rotation(0, std::numbers::pi_v<double> + theta, 0);
