...
struct canDecay {
    static inline constexpr bool value
        = std::is_convertible_v<
              std::decay_t<T>,
              Type> && std::is_convertible_v<std::decay_t<U>, Type2>;
};
...
template <
    ...
    typename = std::enable_if_t<
        !csl::canDecay_v<
            ...
            U,
            csl::
                Index> && !csl::canDecay_v<T, U, csl::IndexStructure> && !csl::canDecay_v<T, U, std::vector<csl::Index>>>>
[[nodiscard]] csl::Expr
Replaced(csl::Expr const &expr, T &&p1, U &&p2, Params &&... params)
{
    csl::Expr cpy = expr;
    csl::Replace(cpy,
                 std::forward<T>(p1),
...
[[nodiscard]] csl::Expr Replaced(csl::Expr const & expr,
                                 csl::Index const &from,
                                 csl::Index const &to,
                                 bool              refresh = true);

inline void Replace(csl::Expr &       expr,
                    csl::Index const &from,
                    csl::Index const &to,
                    bool              refresh = true)
{
 ...
 *
 * @details The function takes a constant expression as parameter and a
 * boolean. If true, this boolean specifies that the rule is used only as a
 * predicate for the moment i.e. that no calculation must be done in the return
 * value (the optional will be used as a bool to know if a replacement must
 * take place). If false, this function must return the result of the
 * replacement in the std::optional<csl::Expr>. If no replacement is possible,
 * return std::nullopt.
 */
...
void internal_RenameIndices(csl::Expr &                       expr,
...
template <class ParentType,
          typename
          = std::enable_if_t<std::is_convertible_v<ParentType, Parent>>>
inline void Replace(Expr &                         expr,
