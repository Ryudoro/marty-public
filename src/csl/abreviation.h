...
class Abbrev {
  ...
  public:
    ...
    struct compareParents {
        bool operator()(AbstractParent *const &A, AbstractParent *const &B);
    };
  ...
  private:
    static void
    compressAbbreviations_impl(std::vector<AbstractParent *> &abbreviations);

    static void addAbbreviation(AbstractParent *   ptr,
                                std::string const &t_name);

    static void removeAbbreviation(AbstractParent *   ptr,
                                   std::string const &t_name);

    static std::string getFinalName(std::string_view initialName);

    static std::vector<AbstractParent *> &
    getAbbreviationsForName(std::string_view name);
  ...
  public:
    ...
    static void printAbbreviations(std::string_view name,
    ...
    getFreeStructure(csl::IndexStructure const &structure);

    static csl::IndexStructure getFreeStructure(Expr const &expr);

    static std::optional<Expr> findExisting(std::string_view name,
                                            Expr const &     encaps);

    static Expr makeSubAbbrev(std::vector<csl::Expr> const &args, bool isProd);

    static Expr makeAbbreviation(std::string name,
    ...
    static void replace(Args &&... args)
    {
        for (auto &el : abbreviationData)
            for (auto &ab : el.second)
                ab->setEncapsulated(csl::Replaced(
                    ab->getEncapsulated(), std::forward<Args>(args)...));
    }
...
class Abbreviation : public BaseParent {
  ...
  public:
    ...
    Abbreviation(Expr const &       t_encapsulated,
                 std::string const &t_name,
                 Args &&... args)
        : BaseParent(Abbrev::getFinalName(t_name),
                         ...
                         bool          header     = false) const override
    {
        ...
        if (header)
            out << "inline ";
        out << "csl::Expr " << regName << " = csl::Abbrev::makeAbbreviation(\""
            << regLite << "\", " << regName << "_encaps);\n";
    }
     ...
     * index structure, from the abbreviation object **self**, to it. The
     * result is then the mathematically equivalent encapsulated expression.
     * Therefore this function performs calculations (contrary to
     * getEncapsulated()) but yields an expression that can replace the initial
     * abbreviation object in expressions ensuring that the result is still
    ...
    csl::Expr getExactEncapsulated(Expr_info self) const override
    {
        ...
        csl::ForEachLeaf(res, [&](Expr &sub) {
            ...
            for (auto &i : index) {
                auto pos = std::find(
                    initialStructure.begin(), initialStructure.end(), i);
    ...
    std::optional<Expr> evaluate(Expr_info       self,
                                 csl::eval::mode user_mode
                                 = csl::eval::base) const override
    {
        if (evaluation
            or csl::eval::isContained(user_mode, csl::eval::abbreviation)) {
