...
class Diagonalizer {
  ...
  public:
    explicit Diagonalizer(const Matrix &matrix,
                          bool          forceDeterminantZero = false);

    explicit Diagonalizer(const Expr &matrix,
                          bool        forceDeterminantZero = false);
  ...
  private:
    ...
    static double extractNumber(const Expr &number);

#ifndef COMPILE_32_BITS
    static gsl_complex csl_to_gsl(const Expr &csl_expr);

    static gsl_matrix_complex *csl_to_gsl(const Matrix &csl_matrix);

    static Expr gsl_to_csl(gsl_complex gsl_expr);

    static Matrix gsl_to_csl(gsl_matrix_complex *gsl_matrix);

    static Matrix diagFromEigenValues(gsl_vector *eigenValues);
#endif
