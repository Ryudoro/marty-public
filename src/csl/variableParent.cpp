...
LiteralParent::LiteralParent(string const &t_name)
    : AbstractParent(t_name), valued(false)
{
}

LiteralParent::LiteralParent(string const &t_name, long double t_value)
    : AbstractParent(t_name), valued(true), value(autonumber_s(t_value))
{
}

LiteralParent::LiteralParent(string const &t_name, Expr const &t_value)
    : AbstractParent(t_name), valued(true), value(t_value)
{
...
void LiteralParent::setValue(Expr const &t_value)
{
    ...
    CSL_ASSERT_SPEC(t_value->getPrimaryType() == csl::PrimaryType::Numerical
                        or t_value == CSL_INF,
                    CSLError::TypeError,
                    "Expecting a numerical type, " + toString(t_value)
                        + " given.");
    value = t_value;
}
...
ConstantParent::ConstantParent(string const &t_name) : LiteralParent(t_name)
{
}

ConstantParent::ConstantParent(string const &t_name, long double t_value)
    : LiteralParent(t_name, t_value)
{
}

ConstantParent::ConstantParent(string const &t_name, Expr const &t_value)
    : LiteralParent(t_name, t_value)
{
}

void ConstantParent::printDefinition(std::ostream &out,
                                     int           indentSize,
                                     bool          header) const
{
    ...
    if (header)
        out << "inline ";
    out << "csl::Expr " << regName << " = csl::constant_s(\"" << regLite
        << "\"";
    if (valued) {
        out << ", ";
...
VariableParent::VariableParent(string const &t_name)
    : LiteralParent(t_name), elementary(false), allDependencies(true)
{
}

VariableParent::VariableParent(string const &t_name, long double t_value)
    : LiteralParent(t_name, t_value), elementary(false), allDependencies(true)
{
}

VariableParent::VariableParent(string const &t_name, Expr const &t_value)
    : LiteralParent(t_name, t_value), elementary(false), allDependencies(true)
{
}

void VariableParent::printDefinition(std::ostream &out,
                                     int           indentSize,
                                     bool          header) const
{
    std::string indent(indentSize, ' ');
    ...
    if (header)
        out << "inline ";
    out << "csl::Expr " << regName << " = csl::variable_s(\"" << regLite
        << '\"';
    if (valued) {
        out << ", ";
        value->printCode(1, out);
    ...
    out << ");\n";
    if (!header) {
        out << indent << regName << "->setElementary(" << elementary << ");\n";
        out << indent << regName << "->setAllDependencies(" << allDependencies
            << ");\n";
    }
    // printPropDefinition(out, indentSize, header);
}
...
bool VariableParent::commutesWith(Expr_info expr, int)
{
    if (commutable) {
