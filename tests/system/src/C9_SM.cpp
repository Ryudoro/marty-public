...
std::vector<size_t> concat(std::vector<std::vector<size_t>> const &vecs)
{
...
std::vector<size_t> indexRange(size_t mini, size_t maxi)
{
...
void select(Amplitude &ampl, std::vector<size_t> const &indices)
{
...
std::vector<size_t> indicesOf(Process p)
{
    switch (p) {
    case Wtt:
        return elements({6, 10, 62, 66});
    case Gtt:
        return elements({7, 8, 9, 11, 12, 13, 63, 64, 65, 67, 68, 69});
    case tWW:
        return elements({0, 1});
    case tGG:
        return indexRange(34, 41);
    case tWG:
        return elements({4, 5});
    case tGW:
        return elements({2, 3});
    case bW:
        return elements({24, 29, 52, 57});
    case sW:
        return elements({14, 19, 42, 43});
    case bG:
        return concat(
            {indexRange(25, 28), indexRange(30, 33), indexRange(53, 56), indexRange(58, 61)});
    case sG:
        return concat(
            {indexRange(15, 18), indexRange(20, 23), indexRange(44, 51), indexRange(53, 56)});
    }
    return {};
}
...
int main()
{
    ...
    return assert_equal("data/output/C9_SM.txt", "libs/C9_SM/data.txt", 1e-5);
}
